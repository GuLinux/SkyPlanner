    ...
    .add(
         WW<WToolBar>()
            ...
            .addButton( WW<WPushButton>("?").css("btn btn-primary").onClick([=](WMouseEvent){
                SkyPlanner::instance()->notifications()->show(WString::tr("help_notification"), WString::tr("help_search_by_name"), Notification::Information, 10 );
            }) )
...
void SelectObjectsWidget::Private::searchByName(const string &name, AstroObjectsTable *table, int page)
{
  ...
  if(tablePage.total > 200) {
    SkyPlanner::instance()->notifications()->show(WString::tr("select_objects_widget_add_by_name"), WString::tr("select_objects_widget_add_by_name_too_many"), Notification::Information, 5);
    return;
  }
