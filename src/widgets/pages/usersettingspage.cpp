...
void UserSettingsPage::Private::onDisplay()
{
  ...
  WPushButton *changePasswordButton = WW<WPushButton>(WString::tr("user_settings_change_password")).css("btn btn-primary").onClick([=](WMouseEvent) {
    ...
    if(hasOldPassword && ! passwordService.verifyPassword(session.login().user(), oldPassword->text() )) {
      Notifications::instance()->show(WString::tr("changepwd_error_title"), WString::tr("changepwd_wrong_password"), Notification::Error, 10);
      return;
    }
    if(newPassword->text() != newPasswordConfirm->text()) {
      Notifications::instance()->show(WString::tr("changepwd_error_title"), WString::tr("changepwd__passwords_not_matching"), Notification::Error, 10);
      return;
    }
    ...
    if( passwordValidation.state() != WValidator::Valid ) {
      Notifications::instance()->show(WString::tr("changepwd_error_title"), passwordValidation.message(), Notification::Error, 10);
      return;
    }
    ...
    Notifications::instance()->show(WString::tr("notification_success_title"), WString::tr("changepwd_passwords_changed"), Notification::Success, 10);
  });
  ...
  changeAddress->clicked().connect([=](WMouseEvent) {
    ...
    Notifications::instance()->show(WString::tr("user_settings_new_email"), WString::tr("user_settings_email_changed_notify") + WString::tr("user_settings_email_verification_notify"), Notification::Information, 10);
  });
  ...
  if(needsVerification) {
    ...
    resendVerification->clicked().connect([=](WMouseEvent) {
      ...
      Notifications::instance()->show(WString::tr("user_settings_email_verification_notify_title"), WString::tr("user_settings_email_verification_notify"), Notification::Information, 10);
    });
