...
void AstroSessionTab::Private::reload()
{
  ...
  if(session.user()->telescopes().size() && session.user()->eyepieces().size()) {
    WTable *instrumentsTable = WW<WTable>().addCss("table table-condensed");
    ...
    instrumentsTable->elementAt(0, 2)->addWidget(WW<WText>(WString::tr("eyepiece")));
    instrumentsTable->elementAt(0, 3)->addWidget(WW<WText>(WString::tr("eyepiece_focal_length")));
    instrumentsTable->elementAt(0, 4)->addWidget(WW<WText>(WString::tr("afov")));
    instrumentsTable->elementAt(0, 5)->addWidget(WW<WText>(WString::tr("magnification")));
    instrumentsTable->elementAt(0, 6)->addWidget(WW<WText>(WString::tr("fov")));
	...
	OpticalSetup setup(telescope, eyepiece);
	...
	row->elementAt(2)->addWidget(WW<WText>(WString::fromUTF8(eyepiece->name())));
	row->elementAt(3)->addWidget(WW<WText>(format("%dmm") % eyepiece->focalLength() ));
	row->elementAt(4)->addWidget(WW<WText>(WString::fromUTF8(eyepiece->aFOV().printable(Angle::IntDegrees))));
	row->elementAt(5)->addWidget(WW<WText>(format("%.2f") % setup.magnification()));
	row->elementAt(6)->addWidget(WW<WText>(WString::fromUTF8(setup.fov().printable())));
      }
    }
